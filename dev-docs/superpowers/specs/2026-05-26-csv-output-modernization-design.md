...
**Decision (approved):** fold buffer-building into `csv_output` so one sampling pass feeds files *and* the live C-API rows (option 1), and **drop the four inert streams** `temperature/solute/surfacewater/agetracer` (option 3) — re-add properly if/when a BMI consumer needs them.
