"""Minimal local ``Driver`` ABC.

Vendored from ``imod_coupler/drivers/driver.py`` (``swap`` branch), trimmed to
the abstract interface the swapmod driver needs and with ``loguru`` swapped for
the stdlib ``logging`` module so the fork imports without extra dependencies.
"""
