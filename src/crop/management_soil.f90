...
!     for experts: apparent age of crop specific residues roots, leaves, shoots, storageOrgans 
      ...
      flCropExt = .false.
      if (RDinqr('flCropExt')) then
         call RDslog ('flCropExt', flCropExt)
         if (flCropExt) then
            filnamce = trim(pathwork)//trim(outfil)//'_crop_ext.csv'
            cropext = getun (20,90)
            call fopens(cropext,filnamce,'new','del')
            write(cropext,'(2a)')'    nishmi     nishma     niromi     niroma     poshmi     poshma     poromi     poroma'
            write(cropext,'(a)')' to be replaced'
 write(cropext,'(2a)')'year-mo-da,Ntotuptake,Ptotuptake,DMcressurf,N-cressurf,P-cressurf,DMcresbott,N-cresbott,P-cresbott'
         end if
      end if
      close(snp)
