!> @file src/solute/dormant/agetracer.f90
!! @brief DORMANT — groundwater-age tracking (Goode 1996).
!!
!! ## Status: DORMANT
!!
!! On 2026-05-24 this file replaces the stub at `src/solute/agetracer.f90`
!! that ADR 0032 had inserted (the stub still lived in `meson.build` and
!! re-exported `AgeTracer` purely to keep two dead `use agetracer_mod`
!! lines in `swap_mod.f90` compiling). With the stub gone, those `use`
!! lines have been dropped, the stub file has been deleted, and the
!! Goode (1996) body is preserved here in full.
!!
!! Why preserve it? `flAgeTracer` was never set `.true.` anywhere in
!! the TOML pipeline (ADR 0032 retired its declaration), but the
!! capability — groundwater age tracking via Goode's ageing equation
!! solved alongside the standard solute transport step — is on the
!! long-term roadmap.
!!
!! Excluded from `meson.build` — this file is NOT compiled. It will
!! also **not compile cleanly** in the current tree: ADR 0032 (commit
!! f1115bf) retired the 22 bare globals it references (the Age*
!! family and dual-use partners). This is deliberate — reactivation
!! must re-home them on state, not restore the stale globals.
!!
!! ## Reactivation checklist (DO NOT skip — defer-until-needed decisions)
!!
!! 1. **Define `agetracer_state_t`**. The 12 AgeTracer-specific globals
!!    originally in `variables.f90` (Ageirr, Agedrain, Agepre, Agepond,
!!    Agepondm1, icAgetopupw, icAgetopdwn, icAgeBot, icAgeDra, icAgeRot,
!!    icAgeSur, AgeGwl1m) are the target field set. Plus the boundary
!!    concentration globals (cpre, cirr, cdrain) that the init branch
!!    reads — those may already have homes in `state%solute`.
!!
!! 2. **Resolve the `cml` dual-use** (discovery hazard). The task=2
!!    finalisation overwrites `state%solute%cml(:)` with age-tracer
!!    concentrations so the existing output channel picks them up.
!!    Either:
!!    (a) Add a separate `state%agetracer%cml_age(:)` field and stop
!!        overwriting `state%solute%cml`; OR
!!    (b) Formalise + document the sequencing rule and keep the
!!        overwrite (faster but more fragile).
!!
!! 3. **Wire `flAgeTracer`** from typed config or an external trigger
!!    (currently never assigned anywhere). Retire the unconditional
!!    stub-error guard at the top of the body below.
!!
!! 4. **Restore the dispatch sites**. Originally in `swap_mod.f90`:
!!    `call AgeTracer(1, state)` from the solute init block, and
!!    `call AgeTracer(2, state)` from the solute step. Both were
!!    removed at ADR 0032 (search for "[SS-GR-CROPRT A1] AgeTracer"
!!    breadcrumbs in `swap_mod.f90`).
!!
!! 5. **Re-add `use agetracer_dormant_mod, only: AgeTracer`** at both
!!    dispatch sites. The original `swap_mod.f90` had these as
!!    `use agetracer_mod, only: AgeTracer` at lines 112 and 487
!!    (dropped 2026-05-24 — see commit log).
!!
!! 6. **Update `outage`'s flAgeTracer guard** in `swapoutput.f90` to
!!    ungate output once the runtime is back online.
!!
!! 7. **Migrate stale globals**. Remove the macropore-retired locals
!!    (`Z_Tp`, `ArMpTp`, `ArMpSs` — ADR 0040 dropped macropores). The
!!    `inpola`/`inpolb` and `ddif`/`ldis` references will need to be
...
end module agetracer_dormant_mod
